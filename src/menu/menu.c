...
void Menu_Load(MenuPage page)
{
	...
	IO_Data sfx_arc = IO_Read("\\SOUNDS\\MENU.VAG;1");

	//Load sound effects
	for (u8 i = 0; i < COUNT_OF(sfx_path); i++)
	{
		menu.sounds[i] = Audio_LoadSFX(Archive_Find(sfx_arc, sfx_path[i]));
	}
	...
	//Fallthrough
		...
		case MenuPage_Story:
		{
			static const struct
			{
				...
				{NULL, StageId_No_Good, "I HATE THAT HEDGEHOG", {"NO GOOD", NULL, NULL}},
			};
		...
		case MenuPage_Freeplay:
		{
			static const struct
			{
				...
				{StageId_No_Good, 0xFF0066CC, "NO GOOD", 1},
				{StageId_No_Good_Old, 0xFF0066CC, "NO GOOD OLD", 2},
			};
			...
			for (u8 i = 0; i < COUNT_OF(menu_options); i++)
			{
				...
				//Draw Icon
				Menu_DrawHealth(menu_options[i].icon, strlen(menu_options[i].text) * 14 + 48 + (y / 6), SCREEN_HEIGHT2 - 16 + y - 8 - 7, menu.select == i);
		...
		case MenuPage_Credits:
		{
			...
			if (menu.next_page == menu.page && Trans_Idle())
			{
				...
				if (menu_options[menu.select - 1].text == NULL && menu.select != 0 && pad_state.press & (PAD_UP))
					skip = -1;

				else if (menu_options[menu.select + 1].text == NULL && menu.select != COUNT_OF(menu_options) - 1 && pad_state.press & (PAD_DOWN))
					skip = 1;
