...
#include "characters/sonic_old.h"
...
static void Stage_LoadNotesPos(void)
{
	...
	else
	{
		for (u8 i = 0; i < 4; i++)
		{
			...
			//Hide the notes (Opponent or Player depending of the mode)
			if (stage.mode != StageMode_2P)
				stage.note_x[(i + 4) ^ stage.note_swap] = FIXED_DEC(400,1);
		}
	}
...
static void Stage_LoadSFX(void)
{
	...
	IO_Data sfx_arc = IO_Read("\\SOUNDS\\STAGE.VAG;1");
	
	for (u8 i = 0; i < COUNT_OF(intro_path); i++)
	{
		stage.introsound[i] = Audio_LoadSFX(Archive_Find(sfx_arc, intro_path[i]));
	}
	...
	sfx_arc = IO_Read("\\SOUNDS\\MENU.VAG;1");

	for (u8 i = 0; i < COUNT_OF(sfx_path); i++)
		stage.sounds[i] = Audio_LoadSFX(Archive_Find(sfx_arc, sfx_path[i]));
...
void Stage_Load(StageId id, StageDiff difficulty, boolean story)
{
	...
	//Load HUD textures
	IO_Data hud_arc = IO_Read("\\HUD\\HUD.ARC;1");
	Gfx_LoadTex(&stage.tex_hud0,  Archive_Find(hud_arc, "hud0.tim"), 0);
	Gfx_LoadTex(&stage.tex_hud1,  Archive_Find(hud_arc, "hud1.tim"), 0);
	Gfx_LoadTex(&stage.tex_intro, Archive_Find(hud_arc, "intro.tim"), 0);
	Mem_Free(hud_arc);
	...
	switch (stage.state)
	{
		...
		{		
  		...
  		Events_Tick();
			...
			//Scroll camera
			if ((stage.chart.cur_section->flag & SECTION_FLAG_OPPFOCUS && stage.opponent != NULL)  || (stage.player == NULL) || (stage.stage_id == StageId_No_Good_Old))
