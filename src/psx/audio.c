...
{
	VAG_Header* sfx_header = (VAG_Header*)sfx_data;
	// subtract size of .vag header (48 bytes), round to 64 bytes
	u32 xfer_size = ((SWAP_ENDIAN(sfx_header->size) - VAG_HEADER_SIZE) + 63) & 0xffffffc0;
	...
	data[SWAP_ENDIAN(sfx_header->size) - 15] = 1; // end + mute
